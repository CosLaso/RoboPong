function collision = pointsInCollision(robot,q,point)

    collision = 0;                         % Start as no collision before we check as we need to have an output

    ellipses = cell(1,5);                  % 5 joints + base
    ellipseParameters = zeros(5,3,5);      % 1st row is centres, 2nd row is radii, 3-5 is rotation matrix
    tr = zeros(4,4,robot.n+1);             % Create rotation matrix for each joint
    ...
    z = (robot.links(i).d + robot.links(i).a);     % Distance in z direction determined by the DH parameters
    radii = [0.08,0.08,z];
    ...
    for i = 1:5                                      % Check collision for all joints (including base)
